...
class SpecialistVelocityCommand(VelocityCommandCommandOnly):
    """Opt-in sampling-only ablation; retains the original mixed distribution.

    Bucket 0 keeps the parent draw; 1 practices reverse; 2/3 practice left/right
    low-speed turning. No changes to observations, rewards, timing or actuators.
    Counts are command draws, not time-weighted occupancy or episode success.
    """

    def __init__(self, cfg, env):
        super().__init__(cfg, env)
        self.sampling_bucket_counts = torch.zeros(4, dtype=torch.long, device=self.device)
    ...
    def _resample_command(self, env_ids):
        ...
        if n == 0:
            return
        weights = torch.tensor(self.cfg.bucket_probabilities, device=self.device)
        bucket = torch.multinomial(weights, n, replacement=True)
        self.sampling_bucket_counts += torch.bincount(bucket, minlength=4)
        for which in (1, 2, 3):
            selected = env_ids[bucket == which]
            if not len(selected):
                continue
            self.vel_command_b[selected] = 0.
            draw = torch.empty(len(selected), device=self.device)
            if which == 1:
                self.vel_command_b[selected, 0] = -draw.uniform_(*self.cfg.backward_speed_range)
            else:
                self.vel_command_b[selected, 2] = draw.uniform_(*self.cfg.turn_speed_range) * (1 if which == 2 else -1)
            self.vel_command_w[selected] = self.vel_command_b[selected]
            self.is_standing_env[selected] = False
            self.is_heading_env[selected] = False
            self.is_world_env[selected] = False
            self.is_forward_env[selected] = False


@_dataclass(kw_only=True)
class SpecialistVelocityCommandCfg(VelocityCommandCommandOnlyCfg):
    bucket_probabilities: tuple[float, float, float, float] = (.5, .25, .125, .125)
    backward_speed_range: tuple[float, float] = (.05, .2)
    turn_speed_range: tuple[float, float] = (.15, .6)

    def __post_init__(self):
        super().__post_init__()
        if len(self.bucket_probabilities) != 4 or any(p < 0 for p in self.bucket_probabilities) or not math.isclose(sum(self.bucket_probabilities), 1.):
            raise ValueError("Expected four nonnegative bucket probabilities summing to 1")
        for lo, hi in (self.backward_speed_range, self.turn_speed_range):
            if not 0 < lo <= hi:
                raise ValueError("Specialist speed magnitudes must be positive and ordered")

    def build(self, env):
        return SpecialistVelocityCommand(self, env)
